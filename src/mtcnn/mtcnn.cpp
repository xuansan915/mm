    ...
    for(int row=0; row<score->height; row++)
    {
        for(int col=0; col<score->width; col++)
        {
            if(*p>Pthreshold)
            {
                ...
                if(bbox.area == 0  || bbox.x1 == 0  || bbox.y1  == 0)
                {
                    continue;
                }
...
Rnet::Rnet()
{
    ...
    //std::cout << modelPath <<   "  "<<filename<< std::endl;
    readData(filename, dataNumber, pointTeam);
...
void mtcnn::findFace(Mat &image)
{
    ...
    for(vector<struct Bbox>::iterator it=firstBbox_.begin(); it!=firstBbox_.end(); it++)
    {
        if((*it).exist)
        {
            ...
            Mat secImage;
            if(temp.x+temp.width > image.cols || temp.y+temp.height > image.rows)
            {
                //std::cout<<"1111111  "<< temp.x <<"  "<< temp.y  <<"  "<< temp.width  <<"  "<< temp.height  <<"  "<<   image.cols  <<"  "<< image.rows<<std::endl;
                continue;
            }
